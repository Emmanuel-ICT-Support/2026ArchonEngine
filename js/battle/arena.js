(function (ns) {
  ...
  function createArena(options) {
    ...
    function preloadBattleSprites() {
      var factionKeys = Object.keys(ns.data.PIECE_DESIGNS);
      var factionIndex;
      var typeIndex;
      var designs;
      var design;

      for (factionIndex = 0; factionIndex < factionKeys.length; factionIndex += 1) {
        designs = ns.data.PIECE_DESIGNS[factionKeys[factionIndex]];

        for (typeIndex = 0; typeIndex < ns.data.PIECE_ORDER.length; typeIndex += 1) {
          design = designs[ns.data.PIECE_ORDER[typeIndex]];
          if (design && design.boardSprite) {
            ensureSpriteImage(design.boardSprite.src);
          }
        }
      }
    }
