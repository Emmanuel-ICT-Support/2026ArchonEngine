(function (ns) {
  ...
  function renderRemnantCommandCard() {
    return (
  ...
  function renderConsensusIntelCard() {
    return (
  ...
  function renderDetailCard(piece, squareState, squareLabel) {
    var design = ns.data.getPieceDesign(piece);
    var faction = ns.data.getFaction(piece.side);
  ...
  function createRenderer() {
    ...
    var gameplayMusicLibrary = ["assets/audio/Gameplay_1.mp3"];
    ...
    function selectGameplayMusicTrack() {
      ...
      if (!gameplayMusicLibrary.length) {
        activeGameplayMusicTrack = "";
        return "";
      }

      if (activeGameplayMusicTrack && gameplayMusicLibrary.indexOf(activeGameplayMusicTrack) !== -1) {
        return activeGameplayMusicTrack;
      }

      nextIndex = Math.floor(Math.random() * gameplayMusicLibrary.length);
      activeGameplayMusicTrack = gameplayMusicLibrary[nextIndex];
      return activeGameplayMusicTrack;
    }

    function syncGameplayMusicTrack(audio, trackSrc) {
      if (!audio || !trackSrc) {
        return;
      }
    ...
    function startGameplayMusic(options) {
      ...
      syncGameplayMusicTrack(audio, trackSrc);
    ...
    function bindEvents() {
      ...
      ethicalChoices.addEventListener("click", function (event) {
        var target = event.target.closest("[data-choice-id]");
        if (!target) {
          return;
        }
      ...
      if (getGameplayMusicAudio()) {
        getGameplayMusicAudio().addEventListener("ended", function () {
          ...
          if (gameplayMusicLibrary.length > 1) {
            activeGameplayMusicTrack = "";
          }
          nextTrack = selectGameplayMusicTrack();
          if (nextTrack) {
            syncGameplayMusicTrack(getGameplayMusicAudio(), nextTrack);
          }
          startGameplayMusic();
        });
      }
